...
class PersonDetailsScreen extends StatefulWidget {
  ...
  const PersonDetailsScreen({Key? key,required this.personData}) : super(key: key);
...
class _PersonDetailsScreenState extends State<PersonDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.personData.name),),
body: ListView(
  padding: EdgeInsets.symmetric(horizontal: 0.01.sw,vertical: 0.012.sh),
  children: [
    ImageView(url: widget.personData.profilePath, width: 0.8.sw, height: 0.8.sw),
    AppConst.sizedBoxH10,
    Text(widget.personData.name,textAlign: TextAlign.center,style: hugeTextBold,),
    AppConst.sizedBoxH5,
    Text(widget.personData.knownForDepartment,textAlign: TextAlign.center,style: bigTextBold,),
    AppConst.sizedBoxH10,
    Consumer<PersonDetailsProvider>(
      builder: (context, provider, child) => StaggeredGridView.countBuilder(
        ...
        itemBuilder: (BuildContext context, int index) => ImageView(url: provider.profileGallery[index].filePath,          width: .45.sw,
          height: .5.sw,downloadableImg: true,),
